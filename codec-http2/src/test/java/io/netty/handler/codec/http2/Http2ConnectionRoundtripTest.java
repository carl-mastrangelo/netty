        ...
        try {
            ...
            runInChannel(clientChannel, new Http2Runnable() {
                ...
                public void run() throws Http2Exception {
                    http2Client.encoder().writeHeaders(ctx(), 3, headers, 0, (short) 16, false, 0,
                            false, newPromise());
                    http2Client.encoder().writeData(ctx(), 3, data.rduplicate(), 0, false, newPromise());
            ...
            runInChannel(clientChannel, new Http2Runnable() {
                ...
                public void run() throws Http2Exception {
                    ...
                    for (int streamId = 3; streamId < upperLimit; streamId += 2) {
                        ...
                        http2Client.encoder().writePing(ctx(), false, pingData.rslice(),
                                newPromise());
                        http2Client.encoder().writeData(ctx(), streamId, data.rslice(), 0,
                                                        false, newPromise());
                        // Write trailers.
                        http2Client.encoder().writeHeaders(ctx(), streamId, headers, 0, (short) 16,
